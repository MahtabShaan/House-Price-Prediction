...
def rmsle_cv(model):
    kf = KFold(n_folds, shuffle=True, random_state=42).get_n_splits(new_train_df.values)
    rmse= np.sqrt(-cross_val_score(model, new_train_df.values, y_train, scoring="neg_mean_squared_error", cv = kf))
    return(rmse)
