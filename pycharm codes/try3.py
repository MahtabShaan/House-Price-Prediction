...
def base_model():
     model = Sequential()
     model.add(Dense(20, input_dim=219, init='normal', activation='relu'))
...
clf = KerasRegressor(build_fn=base_model, nb_epoch=1000, batch_size=5,verbose=2)
clf.fit(X_train, y_train)

#make predictions and create the submission file
kpred = clf.predict(X_test)
kpred = np.exp(kpred)
pred_df = pd.DataFrame(kpred, index=test_ID, columns=["SalePrice"])
pred_df.to_csv('keras2.csv', header=True, index_label='Id')